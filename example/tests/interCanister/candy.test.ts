...
describe("when calling Motoko canister from Rust canister it", () => {
    ...
    test("should accept Blob ", async () => {
        ...
        expect(candyBlob.Blob).toEqual(new Uint8Array([0, 0, 0, 0, 0, 48, 0, 211, 1, 1]));
    });
    ...
    test("should accept Bytes ", async () => {
        ...
        expect(candyBytes.Bytes).toEqual(new Uint8Array());
    });
    ...
    test("should accept Map ", async () => {
        const candyMap: CandyShared = await interCanisterCallByRust<CandyShared>(
            "getCandyMap"
        );
        expect(candyMap).toEqual({'Map': [["icp", {Nat: 15n}]]});

        const valueMap: CandyMap = await interCanisterCallByRust<CandyMap>(
            "getCandyMap"
        );
        expect(valueMap.Map).toEqual([["icp", {Nat: 15n}]])
    });

    //ValueMap
    test("should accept ValueMap ", async () => {
        const candyMap: CandyShared = await interCanisterCallByRust<CandyShared>(
            "getCandyValueMap"
        );
        expect(candyMap).toEqual({'ValueMap': [[{Nat: 15n}, {Nat: 15n}]]});

        const valueMap: CandyValueMap = await interCanisterCallByRust<CandyValueMap>(
            "getCandyValueMap"
        );
        expect(valueMap.ValueMap).toEqual([[{Nat: 15n}, {Nat: 15n}]])
    });
    ...
    const interCanisterCallByRust = async <T>(methodName: string) => {
        const val: CandyShared = await rustCaller.get_candy(
            canister_ids.candyFunctions.local,
            methodName
        );
        return val as T;
    };
